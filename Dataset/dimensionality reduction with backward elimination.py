...
dataset = pd.read_csv('C:\\Works\\NSU\\CSE419\\repo\\CSE419-Sensor-Data-based-Activity-Classification\\Dataset\\dataset.csv')
...
pd.DataFrame(X).to_csv("E:\\temporary working files\\Section 5 - Multiple Linear Regression\\vulnerability test\\preprocessed300peopledata.csv")
...
X = np.append(arr = np.ones((28924,1)).astype(int), values = X, axis = 1)
X_opt = X
